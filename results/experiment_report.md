...
| C | 定长序列（压缩） | 28 维全局+分段+转移特征 | RF / XGBoost |
...
| 资源受限设备 | Group C XGB | 特征固定28维，小模型 |
...
4. **参数选择**：窗口大小 W=300s、步长 S=W 为默认最优；序列长度 H=50 最优（四模型均值宏-F1 最高），步长 L=10 为 RNN 最佳（LSTM 宏-F1=0.9973），但特征构造时间随 L 减小显著增长（L=10 时约 160–184s vs L=50 时约 28s）。
